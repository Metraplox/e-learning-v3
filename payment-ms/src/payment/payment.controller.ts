...
export class PaymentController {
  ...
  async createTransaction(@Payload() amount: number) {
    console.log('Received create transaction request with amount:', amount);
  ...
  @MessagePattern('confirm-transaction')
  async confirmTransaction(@Payload() token: string) {
    console.log('Received confirm transaction request with token:', token);
    const tx = new WebpayPlus.Transaction(
      new Options(
        IntegrationCommerceCodes.WEBPAY_PLUS,
        IntegrationApiKeys.WEBPAY,
